...
import { OnChainReportSchema } from '../types/schemas.js';
const SYSTEM_PROMPT = `
...
`;
export async function runOnChainAnalyst(domain) {
    console.log(`[OnChain Analyst] Starting analysis for ${domain}...`);
    const rawData = await getDomaOnChainData(domain);
    if (!rawData) {
    ...
    const transferActivities = rawData.tokens?.flatMap((token) => token.activities?.filter((act) => act.__typename === 'TokenTransferredActivity') || []) || [];
    // CORRECTED: Added 'any' type to act parameter
    const renewalActivities = rawData.activities?.filter((act) => act.__typename === 'NameRenewedActivity') || [];
    ...
    const chainHistory = rawData.tokens?.map((token) => token.networkId) || [];
